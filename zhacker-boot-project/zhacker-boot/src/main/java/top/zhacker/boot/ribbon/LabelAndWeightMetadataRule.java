		...
		for (Server server : serverList) {
			...
			if (!StringUtils.isEmpty(labelOr)) {
				List<String> metadataLabel = Arrays
						.asList(labelOr.split(LabelHeaderMvcHandlerInterceptor.HEADER_LABEL_SPLIT));
				for (String label : metadataLabel) {
					if (LabelHeaderMvcHandlerInterceptor.label.get().contains(label)) {
						return server;
					}
				}
			}
			...
			if (!StringUtils.isEmpty(labelAnd)) {
				List<String> metadataLabel = Arrays
						.asList(labelAnd.split(LabelHeaderMvcHandlerInterceptor.HEADER_LABEL_SPLIT));
				if (LabelHeaderMvcHandlerInterceptor.label.get().containsAll(metadataLabel)) {
					return server;
				}
			}
